patches-own [
  ...
  obs?                 ;; true on obstacle, false elsewhere
]
...
to setup-chemical
  ...
  while [ a <= 0.8 ]
   [  set a a + 0.05
      if (distancexy (-1 * a * max-pxcor) (a * max-pycor)) < 2
      [ set chemical 5 ]
      if (distancexy (-1 * a * max-pxcor) (a * max-pycor)) < 3 and chemical = 0
      [ set chemical 2 ]
  ]
end


to setup-spe-chemical
  set a 0
  while [ a <= 0.8 ]
   [  set a a + 0.05
      if (distancexy (-1 * a * max-pxcor) (a * max-pycor)) < 2
      [ set spe-chemical 2 ]
      if (distancexy (-1 * a * max-pxcor) (a * max-pycor)) < 3 and spe-chemical = 0
      [ set spe-chemical 1 ]
  ]
end

to setup-obstacle  ;; patch procedure
  ;; set obs? variable to true inside the nest, false elsewhere
  ;;ask patches [
    set obs? (distancexy (-0.4 * max-pxcor) (0.4 * max-pycor)) < 2 ;;]
end
...
to recolor-patch  ;; patch procedure
  ...
  [ ifelse food > 0
    ...
    [ ifelse chemical > 0
      ...
      [ set pcolor scale-color sky spe-chemical 0.1 3 ] ] ]
  if obs?  [set pcolor lime]
...
to go  ;; forever button
  ask turtles
  [ if seqOut [
      if who >= ticks * 5 [ stop ] ] ;; delay initial departure
    ifelse color = red
  ...
  diffuse spe-chemical (diffusion-rate / 100)
  ask patches
  [
    set chemical chemical * (100 - evaporation-rate) / 100  ;; slowly evaporate chemical
    setup-spe-chemical
    recolor-patch
    if add and ticks = 300 and (distancexy (-0.6 * max-pxcor) (-0.6 * max-pycor)) < 4
       [ set food-source-number 2
         set food 2 ]]
  print-result
  tick
...
to print-result
  file-open "Ants_h.txt"
  file-print ticks
  ask turtles
  [ file-type xcor file-write ycor file-write who file-print ""]
  file-close
end
...
to look-for-food  ;; turtle procedure
  ...
  [ uphill-chemical ]
  if who < header-num and (spe-chemical >= 0.05) and (spe-chemical < 2)
  [ uphill-spe-chemical ]
end

to uphill-spe-chemical  ;; turtle procedure
  let scent-ahead spe-chemical-scent-at-angle   0
  let scent-right spe-chemical-scent-at-angle  45
  let scent-left  spe-chemical-scent-at-angle -45
  ifelse scent-ahead = -1 [ rt 180 ]
...
to wiggle  ;; turtle procedure
  ;;let angle one-of [45 0 -45]
  ;;let angle random 45
  ;;let p patch-right-and-ahead angle 1
  ;; if  [ rt 180 ]
  ;;ifelse p = nobody or [obs?] of p = True [ rt 180 ]
  ;;[ rt angle
  ;;  lt angle]
  rt random 40
  lt random 40
...
to-report spe-chemical-scent-at-angle [angle]
  let p patch-right-and-ahead angle 1
  if p = nobody [ report 0 ]
  if [obs?] of p = True [ report -1 ]
  report [spe-chemical] of p
end
@#$#@#$#@
GRAPHICS-WINDOW
407
